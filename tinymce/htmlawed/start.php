...
function htmlawed_hook($element, $attribute_array = false) {
	...
	$allowed_styles = trigger_plugin_hook('allowed_styles', 'htmlawed', NULL, $allowed_styles);
...
function htmlawed_filter_tags($hook, $entity_type, $returnvalue, $params) {
	...
	if (!isadminloggedin()){
		...
		if (include_once(dirname(__FILE__) . "/vendors/htmLawed/htmLawed.php")) {
			...
			$htmlawed_config = trigger_plugin_hook('config', 'htmlawed', NULL, $htmlawed_config);
